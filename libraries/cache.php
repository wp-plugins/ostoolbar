...
class OST_Cache {
	...
	public static function callback($object, $method, $args=array(), $cache_lifetime=null) {
		...
		$cache = OST_Factory::getInstance('JCacheStorageFile');
		
		if (!$cache_lifetime)
		{
			$cache_lifetime = self::HALF_DAY;
		}
		
		$cache->setLifetime($cache_lifetime);

		//$cache->remove($cache_id, self::CACHE_GROUP);
		//die();
		...
		if ($response->hasError())
		{
			...
			$key_error = true;
			self::$cache_group = self::$cache_group."_trial";
			OST_RequestHelper::isTrial();

		}
		
		$callback	= array($object, $method);
		$cache_id = self::makeCacheId($callback, $args);
		
		$data = $cache->get($cache_id, self::$cache_group);
		
		if ($data)
		{
			$data = unserialize($data);
			$response = OST_RequestHelper::makeRequest(array('resource' => 'lastupdate'));
			if (!$response->hasError())
			{
				...
				if (is_array($data))
				{
					if ((count($data) &&  strtotime($data[0]->last_update_date) < $last_update) || count($data) == 0)
					{
						...
						if ($data !== false)
						{
							...
							$cache->store($cache_id, self::$cache_group, $cached);
						}
						return $data;
					}
				}
	...
	public function makeCacheId($callback, $args)
	{
		...
		return md5(serialize(array($callback, $args, self::$cache_group)));
	}
